...
import { BigNumber, Contract, Signer, Wallet, keccak256, AbiCoder, toBeArray, toBeHex } from 'ethers'
...
export function signUserOp (op: UserOperation, signer: Wallet, entryPoint: string, chainId: number): UserOperation {
  ...
  const msg1 = Buffer.concat([
    ...
    Buffer.from(toBeArray(message))
  ])

  const sig = ecsign(keccak256_buffer(msg1), Buffer.from(toBeArray(signer.privateKey)))
...
export async function fillUserOp (op: Partial<UserOperation>, entryPoint?: EntryPoint, getNonceFunction = 'getNonce'): Promise<UserOperation> {
  ...
  if (op.initCode != null) {
    const initAddr = ethers.utils.hexDataSlice(op1.initCode!, 0, 20)
    const initCallData = ethers.utils.hexDataSlice(op1.initCode!, 20)
    ...
    if (op1.sender == null) {
      ...
      if (initAddr.toLowerCase() === Create2Factory.contractAddress.toLowerCase()) {
        const ctr = ethers.utils.hexDataSlice(initCallData, 32)
        const salt = ethers.utils.hexDataSlice(initCallData, 0, 32)
